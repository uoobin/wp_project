...
var mongoose   = require('mongoose'); //mongoose middleware 로드
...
const connStr = 'mongodb://YooBin:rkwkrhrh5#@ds229373.mlab.com:29373/web'; //mongodb atlas connection string 가져 와서 넣어도 됨
// 아래는 mLab을 사용하는 경우의 예: 본인의 접속 String으로 바꾸세요.
// const connStr = 'mongodb://dbuser1:mju12345@ds113825.mlab.com:13825/sampledb1';
mongoose.connect(connStr, {useMongoClient: true }); //mongodb와 연결!
mongoose.connection.on('error', console.error); //에러 시 출력

// Favicon은 웹사이트의 대표 아이콘입니다. Favicon을 만들어서 /public에 둡시다.
// https://www.favicon-generator.org/ 여기서 만들어볼 수 있어요.
app.use(favicon(path.join(__dirname, 'public', 'favicon.ico'))); //favicon:웹사이트 티아틀 페이지에 나오는 작은 아이콘
app.use(logger('dev'));
...
app.use(methodOverride('_method', {methods: ['POST', 'GET']})); //get이나 post로 들어온 메소드들을 _method를 통해서 변경할 수 있도록

// session을 사용할 수 있도록.
app.use(session({ //접속한 사람 계속 추적
  resave: true,
